...
plt.plot(epochs, GenE, 'k', label='5 layer NN')
#plt.scatter(epochs,GenE,c=epochs,s=20, cmap = 'viridis')
plt.plot(epochs3,GenE3, 'k--', label='3 layer NN')
#plt.scatter(epochs3,GenE3,c=epochs,s=20, cmap = 'cool')
#plt.plot(epochs3, GenE1, 'r:')
#plt.scatter(epochs3,GenE1,c=epochs,s=20, cmap = 'Wistia')
plt.xlabel('Epochs')
...
plt.subplot(1,3,2)
plt.scatter(np.squeeze(IXT3),np.squeeze(ITY3), c=GenE3, s=20, cmap='cool')
plt.xlabel('I(X;T)')
plt.ylabel('I(T;Y)')
cbar = plt.colorbar()
#cbar.set_label('Gen Gap')
plt.title('3 layer NN Information')
plt.xlim([0,5])
plt.subplot(1,3,3)
plt.scatter(np.squeeze(IXT),np.squeeze(ITY), c=GenE, s=20, cmap='viridis')
plt.xlabel('I(X;T)')
#plt.ylabel('I(T;Y)')
plt.colorbar()
plt.title('5 layer NN Information')
plt.xlim([0,5])
plt.show()
