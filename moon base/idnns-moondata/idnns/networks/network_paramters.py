...
def get_default_parser(num_of_samples=None):
	...
	parser.add_argument('-learning_rate',
	                    '-l', dest="learning_rate", default=0.0004, #changing from 0.0004
	                    type=float,
	                    help='The learning rate of the network')

	parser.add_argument('-num_repeat',
	                    '-r', dest="num_of_repeats", default=1,  # changed from 1
	...
	parser.add_argument('-net',
	                    '-n', dest="net_type", default='5', #NETWORK ACHERITCTURE
	...
	parser.add_argument('-num_samples',
	                    '-ns', dest="num_of_samples", default=400,    #we want this to be 400 for the info pla
	                    type=int,
	...
	parser.add_argument('-save_grads',
	                    '-sgrad', dest="save_grads", type=str2bool, nargs='?', const=True, default=True,  # changed from false
...
def select_network_arch(type_net):
	...
	if type_net == '1':
		layers_sizes = [[100, 70, 50, 40, 30]]
	elif type_net == '1-2-3':
		layers_sizes = [[10, 9, 7, 7, 3], [10, 9, 7, 5, 3], [10, 9, 7, 3, 3]]
	elif type_net == '11':
		layers_sizes = [[10, 7, 7, 20, 3]] #changed from 4
	elif type_net == '2':
