...
class informationNetwork():
	...
	def __init__(self, rand_int=0, num_of_samples=None, args=None):
		...
		self.epochs_indexes = np.unique(np.logspace(np.log2(args.start_samples), np.log2(args.num_ephocs), args.num_of_samples, dtype=int, base=2)) - 1 # MSB note: this is what determines the number of samples!
		max_size = np.max([len(layers_size) for layers_size in self.layers_sizes])
		# load data
		#np.random.seed(0)
		#fig = plt.figure(figsize=(5,5))
		#X, y = datasets.make_moons(1000, noise=0.2) #noise up from 0.2
		#Xtest, ytest = datasets.make_moons(1000, noise=0.2)
		#self.data_sets.data = X
		#self.data_sets.labels = y
        #test.data = Xtest
        #test.labels = ytest
		self.data_sets = load_data(self.name, args.random_labels) # moon data
		#plt.scatter(self.data_sets.data[:,0], self.data_sets.data[:,1])
		#plt.show()
		# create arrays for saving the data
		self.ws, self.grads, self.information, self.informationError, self.models, self.names, self.networks, self.weights = [
		...
		self.loss_train, self.loss_test, self.test_error, self.train_error, self.l1_norms, self.l2_norms = \
			[np.zeros((self.num_of_repeats, len(self.layers_sizes), len(self.train_samples), len(self.epochs_indexes)))
			 for _ in range(6)]
		self.testerrorsample = np.zeros(np.int(np.floor(self.num_ephocs/self.interval_accuracy_display)))
		self.trainerrorsample = np.zeros(np.int(np.floor(self.num_ephocs/self.interval_accuracy_display)))
		...
		self.interval_accuracy_display = 50 #MSB addition to override initialization
	...
	def save_data(self, parent_dir='jobs/', file_to_save='data.pickle'):
		...
		data = {'information': self.information, 'informationError':self.informationError,
			...
			, 'l1_norms': self.l1_norms, 'weights': self.weights, 'ws': self.ws} #MSB added informationError
	...
	def run_network(self):
		...
		self.testerrorsample = current_network['test_prediction']
		self.trainerrorsample = current_network['train_prediction']
		self.informationError = current_network['informationError']

	def returntesterror(self, network):
	#MSB: needs the network to be fed in.
		for i in range(len(self.train_samples)):
			for j in range(len(self.layers_sizes)):
				for k in range(self.num_of_repeats):
					index = i * len(self.layers_sizes) * self.num_of_repeats + j * self.num_of_repeats + k
	...
	def plot_network(self):
		...
		mode = 2
		save_name = 'figuretanhNet10x_tanh'
		plt_fig.plot_figures(str_names, mode, save_name)
		#plt_fig.main()
		#plt_fig.plot_hist(str_names)
