...
temp = np.load('Ixt200_{}.npy'.format(0))
#print(temp.shape)
IXT = np.zeros(temp.shape)
ITY = np.zeros(temp.shape)
epochs = np.load('epochcount200.npy')
temp = np.load('Testerror200_{}.npy'.format(0))/20
TrainE = np.zeros(temp.shape)
TestE = np.zeros(temp.shape)
#print(np.arange(78).shape)
#print(np.squeeze(IXT).shape)
for i in range(20):
	IXT += np.load('Ixt200_{}.npy'.format(i))/20
	ITY += np.load('Ity200_{}.npy'.format(i))/20
	TestE += np.load('Testerror200_{}.npy'.format(i))/20
	TrainE += np.load('Trainerror200_{}.npy'.format(i))/20
	
temp = np.load('Ixt3_{}.npy'.format(0))
#print(temp.shape)
IXT3 = np.zeros(temp.shape)
ITY3 = np.zeros(temp.shape)
epochs3 = np.load('epochcount3.npy')
temp = np.load('Testerror3t_{}.npy'.format(0))/20
TrainE3 = np.zeros(temp.shape)
TestE3 = np.zeros(temp.shape)
#print(np.arange(78).shape)
#print(np.squeeze(IXT).shape)
for i in range(20):
	IXT3 += np.load('Ixt3_{}.npy'.format(i))/20
	ITY3 += np.load('Ity3_{}.npy'.format(i))/20
	TestE3 += np.load('Testerror3t_{}.npy'.format(i))/20
	TrainE3 += np.load('Trainerror3t_{}.npy'.format(i))/20

temp = np.load('Ixt1_{}.npy'.format(0))	
IXT1 = np.zeros(temp.shape)
ITY1 = np.zeros(temp.shape)
epochs1 = np.load('epochcount1.npy')
temp = np.load('Testerror1_{}.npy'.format(0))/20
TrainE1 = np.zeros(temp.shape)
TestE1 = np.zeros(temp.shape)
#print(np.arange(78).shape)
#print(np.squeeze(IXT).shape)
for i in range(20):
	IXT1 += np.load('Ixt1_{}.npy'.format(i))/20
	ITY1 += np.load('Ity1_{}.npy'.format(i))/20
	TestE1 += np.load('Testerror1_{}.npy'.format(i))/20
	TrainE1 += np.load('Trainerror1_{}.npy'.format(i))/20
...
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.title('200(-), 3(--) and 1(:) Hidden Neurons Accuracy')
...
plt.ylabel('I(T;Y)')
plt.colorbar()
plt.title('200(viridis), 3(cool) and 1(Wistia) Hidden Neurons Information')
plt.show()
