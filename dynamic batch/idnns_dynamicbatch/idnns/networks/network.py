...
def train_and_calc_inf_network(i, j, k, layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
	...
	if calc_information:
		...
		#print ('Calc info at ErrorValues')
		#infomrationError = np.array([inn.get_information(network['ws'], data_sets_org.data, data_sets_org.labels,
		                                            #num_of_bins, interval_accuracy_display, network['model'],
		                                            #layerSize)])
		network['informationError'] = infomration
		#print(network['informationError'])
...
def print_accuracy(batch_points_test, data_sets, model, sess, j, acc_train_array):
	...
	return {'test':np.mean(np.array(acc_array)), 'train':np.mean(np.array(acc_train_array))}
...
def train_network(layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
	...
	cts = 0
	flip = 1
	testerrorsample = np.zeros(len(indexes)) #np.zeros(np.int(np.floor(num_of_ephocs/interval_accuracy_display)))
	trainerrorsample = np.zeros(len(indexes))
	with tf.Session() as sess:
		...
		for j in range(0, num_of_ephocs):
			...
			# Print accuracy
			indexesnew = indexes
			if j in indexesnew: #np.mod(j, interval_accuracy_display) == 1 or interval_accuracy_display == 1:
				temperror = print_accuracy(batch_points_test, data_sets, model, sess, j, acc_train_array)
				testerrorsample[cts] = temperror['test']
				trainerrorsample[cts] = temperror['train']
				cts+= 1
			if j in indexes:
				if save_grads:
				...
				# saver.save(sess, model.save_file, global_step=k, write_meta_graph=write_meta) #is this where i need to write out to get the gradients?
				k += 1
			flip = 2
			if j%800 == 799 and flip == 1:
				batch_size = np.min([2048, data_sets.train.data.shape[0]])
				batch_points = np.rint(np.arange(0, data_sets.train.data.shape[0] + 1, batch_size)).astype(dtype=np.int32)
				batch_points_test = np.rint(np.arange(0, data_sets.test.data.shape[0] + 1, batch_size)).astype(dtype=np.int32)
				batch_points_all = np.rint(np.arange(0, data_sets_org.data.shape[0] + 1, batch_size)).astype(dtype=np.int32)
				if data_sets_org.data.shape[0] not in batch_points_all:
				...
				if data_sets.test.data.shape[0] not in batch_points_test:
					batch_points_test = np.append(batch_points_test, [data_sets.test.data.shape[0]])
				flip = 2
			if j%800 == 799 and flip == 0:
				batch_size = np.min([128, data_sets.train.data.shape[0]])
				batch_points = np.rint(np.arange(0, data_sets.train.data.shape[0] + 1, batch_size)).astype(dtype=np.int32)
				batch_points_test = np.rint(np.arange(0, data_sets.test.data.shape[0] + 1, batch_size)).astype(dtype=np.int32)
				batch_points_all = np.rint(np.arange(0, data_sets_org.data.shape[0] + 1, batch_size)).astype(dtype=np.int32)
				if data_sets_org.data.shape[0] not in batch_points_all:
	...
	network = {}
	network['ws'] = ws
	network['test_prediction'] = testerrorsample#test_prediction
	#print(testerrorsample)
	network['train_prediction'] = trainerrorsample #train_prediction
